...
\end{split}
\end{equation}
Будем искать неизвестную функцию в виде комбинации из $N$ гармоник:
\begin{equation}\label{3_spect_approx}
u(x, t) = \sum_{k=-N/2+1}^{N/2} \widehat u(k, t) e^{ikx}, \quad x\in(0, 2\pi).
\end{equation}
%Основная идея метода заключается в подстановке \eqref{3_spect_approx} в уравнение \eqref{3_bq_reg_scaled}, решение его в пространстве Фурье $\widehat{u}$ и 
...
\label{3_idft}
u(x_j, t) &= \mathcal{F}^{-1} \widehat u = \sum_{k=-N/2+1}^{N/2} \widehat u(k, t) e^{ikx_j}, \quad x_j = \frac{2\pi j}{N-1}, \quad j=0, \dots, N-1.
\end{align}
Применим ДПФ к уравнению \eqref{3_bq_reg_scaled} и начальным условиям \eqref{3_bq_iv}. Приравнивая коэффициент при каждой гармонике к нулю, получим систему ОДУ с начальными условиями в виде:
...
Для нахождения Фурье образа нелинейного слагаемого $u^2$, а также $uP$, к Фурье образу $\widehat u$ применяется обратное преобразование, затем полученная функция $u(x_j, t)$ возводится в квадрат в узлах сетки или домножается на $P(x_j, t)$ и переводится обратно в пространство Фурье:
$$
...
Разрабатывая численную схему для решения полных уравнений, мы задались целью сделать её пригодной для как можно более широкого круга численных экспериментов. Во-первых, схема должна подходить для моделирования эволюции некоторого начального импульса (решение задачи Коши на периодическом интервале). Во-вторых, должна быть возможность моделировать волны, возникающие в результате некоторого внешнего воздействия ...

Рассмотрим осесимметричный стержень круглого сечения длиной $L$ и радиусом $R$.
...
&\tens{E} = \frac12 \lb \nabla\vect{U}^T + \nabla\vect{U} + \nabla\vect{U}^T \cdot \nabla\vect{U} \rb,
\end{align}
где $\tens{E}^*$ -- союзная матрица, а $\mathcal{I}$ -- единичный тензор.
...
\begin{figure}[h]
	...
	\caption{Сравнение решений задачи Коши~\eqref{3_ic_u}, \eqref{3_ic_v} для полной модели и задачи Коши~\eqref{3_bq_ic} для упрощённой модели типа Буссинеска.}
