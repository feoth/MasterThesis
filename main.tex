...
Однако солитоны такой амплитуды вызывают напряжения близкие к пределу упругости для полистирола. В экспериментах с полистироловым стержнем, описанных в~\cite{Garbuzov}, амплитуда очень мала: $A \sim 10^{-3} - 10^{-4}$, следовательно, в главном порядке по $A$ во всех четырёх формулах параметр длины примерно равен
...
\subsection{Образование солитона из удара по поверхности}
...
\begin{enumerate}
	\item Выведены две новые асимптотические модели типа Буссинеска, описывающие длинные продольные волны в стержнях круглого сечения. Эти модели обобщены на случай ненулевой осесимметричной нагрузки на боковой поверхности, а также на случай предварительно растянутого стержня. Установлено, что новые модели:
	\begin{itemize}[noitemsep,topsep=1pt]
	...
	\item Построен метод, позволяющий численно моделировать полные трёхмерные уравнения. В серии численных экспериментов показано, что:
	\begin{itemize}[noitemsep,topsep=1pt]
		\item в полной модели, как и в модели Буссинеска, формируются солитоны;
		\item чем меньше амплитуда солитона, тем более схожи его свойства в полной модели и модели Буссинеска, а именно, зависимость его скорости от амплитуды;
		\item солитоны деформации могут быть возбуждены с помощью воздействия на поверхность стержня (удара), причём модель Буссинеска даёт схожие с полной моделью солитоны.
	\end{itemize}
\end{enumerate}
