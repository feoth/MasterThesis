...
Разобьём область задачи на несколько доменов вдоль оси $x$ и на каждом домене будем искать решение в некотором конечномерном пространстве, задаваемым системой базисных функций $\Phi_{n_1}(x), \Psi_{n_2}(r), \Theta_{n_3}(\varphi)$. Представим приближённое решение на $i$-м домене $\vect{U}_{N}^{(i)}$ в спектральном виде и потребуем, чтобы приближённое решение совпадало с точным в точках коллокации:
\begin{align}
\label{3_spect_approx_full}
&\vect{U}_{N}^{(i)}(x,r,\varphi,t) = \sum_{n_1,n_2,n_3} \vect{\widehat{U}}^{(i)}(n_1,n_2,n_3,t) \Phi_{n_1}(x) \Psi_{n_2}(r) \Theta_{n_3}(\varphi),\\
\label{3_pseudo_sp_approx_full}
\begin{split}
&\vect{U}_{N}^{(i)}(x_{j_1}, r_{j_2}, \varphi_{j_3}, t) = \vect{U}(x_{j_1}, r_{j_2}, \varphi_{j_3}, t), \quad j_1 = 0,\dots, N_1-1, \quad j_2 = 0,\dots, N_2-1,\\
&\hspace{72.5mm} j_3 = 0,\dots, N_3-1.
\end{split}
\end{align}
Подстановка \eqref{3_spect_approx_full} в уравнения \eqref{3_eqns_full} приводит к системе ОДУ второго порядка относительно $\vect{\widehat{U}}^{(i)}$ на каждом из доменов:
\begin{equation}\label{3_ode_domain}
\vect{\ddot{\widehat U}}_{N,k}^{(i)} = F_{k}^{(i)}(\vect{\widehat U}_{N,k_0}, \dots, \vect{\widehat U}_{N,k_N}), \quad k = k_0, \dots, k_N.
\end{equation}
Здесь $k$ -- мультииндекс: $k = (j_1,j_2,j_3)$, $k_0=(0,0,0)$, $k_N = (N_1-1,N_2-1,N_3-1)$, а также использовано обозначение: $\vect{\widehat U}_{N,j_1,j_2,j_3}^{(i)} = \vect{\widehat U}_{N}^{(i)}(x_{j_1}, r_{j_2}, \varphi_{j_3}, t)$. Следующая задача -- сшивка доменов. 

Для обработки нелинейных слагаемых необходимо иметь быстрое преобразование для перехода из физического пространства (пространства функций, заданных своими значениями на сетке) в спектральное пространство...
